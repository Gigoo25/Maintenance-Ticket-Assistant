...
if exist "%root%\Tools\Version.txt" (
	...
	set /p CURRENT_VERSION=<%root%\Tools\Version.txt
) else (
...
if "%Version_Online%" GTR "%Version_Local%" (
	REM Update tool based on variables
	"%root%\Tools\WGET\wget.exe" -q  --retry-connrefused --waitretry=1 --read-timeout=20 --timeout=15 -t 2 "%REPO_URL%/%REPO_BRANCH%/Tools/Version.txt" -O "%root%\Tools\Version.txt"
)

REM Update Batch Script
if "%Batch_Script_Online%" GTR "%Batch_Script_Local%" (
	REM Update tool based on variables
	"%root%\Tools\WGET\wget.exe" --retry-connrefused --waitretry=1 --read-timeout=20 --timeout=15 -t 2 --progress=bar:force "%REPO_URL%/%REPO_BRANCH%/RunMe.bat" -O "%root%\RunMe.bat"
)

REM Update Python Script
if "%Python_Script_Online%" GTR "%Python_Script_Local%" (
	REM Update tool based on variables
	"%root%\Tools\WGET\wget.exe" --retry-connrefused --waitretry=1 --read-timeout=20 --timeout=15 -t 2 --progress=bar:force "%REPO_URL%/%REPO_BRANCH%/script.py" -O "%root%\script.py"
)

REM Update Requirements
if "%Requirements_Online%" GTR "%Requirements_Local%" (
	REM Update tool based on variables
	"%root%\Tools\WGET\wget.exe" --retry-connrefused --waitretry=1 --read-timeout=20 --timeout=15 -t 2 --progress=bar:force "%REPO_URL%/%REPO_BRANCH%/Tools/Requirements.txt" -O "%root%\Tools\Requirements.txt"
)

REM Update "Update Function"
if "%Update_Function_Online%" GTR "%Update_Function_Local%" (
	REM Update tool based on variables
	"%root%\Tools\WGET\wget.exe" --retry-connrefused --waitretry=1 --read-timeout=20 --timeout=15 -t 2 --progress=bar:force "%REPO_URL%/%REPO_BRANCH%/Tools/Functions/Update_Function.bat" -O "%root%\Tools\Functions\Update_Function.bat"
)
